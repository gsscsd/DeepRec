...
class BaseModel(object):
    def __init__(self, dense_feature, sparse_feature, task=None):
        self._dense_inp = {
            feat.name: Input(shape=(feat.dim,),
                             name=feat.name, dtype=feat.dtype)
    ...
    def _build(self):
        ...
        sparse_inp = tf.concat([self.embed_layers[feat](inp)
                                for feat, inp in self._sparse_inp.items()], axis=-1)
        sparse_inp = tf.squeeze(sparse_inp, axis=1)
        inp = tf.concat([dense_inp, sparse_inp], axis=-1)
        out = Dense(units=1, activation='sigmoid')(inp)
        self._model = tf.keras.models.Model(inputs=list(
            self._dense_inp.values()) + list(self._sparse_inp.values()), outputs=out)
